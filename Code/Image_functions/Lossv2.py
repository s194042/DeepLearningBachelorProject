...
class Stem(nn.Module): # Based very loosly on inception v4
    def __init__(
        ...
        self.skip_1x1_1 = nn.Conv2d(channels,64, kernel_size=1, padding=0, bias=True)
        self.skip_1 = nn.Conv2d(64 if self.seperable else channels,64, kernel_size=7, padding=2, stride=4, groups=64 if self.seperable else 1, bias=True)
        self.skip_slim_7x1_1 = nn.Conv2d(64 if self.seperable else channels,64, kernel_size=(7,1), padding=(3,0), stride=(4,1), groups=64 if self.seperable else 1, bias=True)
        self.skip_slim_1x7_1 = nn.Conv2d(64,64, kernel_size=(1,7), padding=(0,3), stride=(1,4), groups=64 if self.seperable else 1, bias=True)
        

        self.base_1x1 = nn.Conv2d(channels,32, kernel_size=1, padding=0, bias=True)
        self.base_3x3 = nn.Conv2d(32 if self.seperable else channels,32, kernel_size=3, padding=1, stride=2, groups=32 if self.seperable else 1, bias=True)
        self.base_slim_3x1 = nn.Conv2d(32 if self.seperable else channels,32, kernel_size=(3,1), padding=(1,0), stride=(2,1), groups=32 if self.seperable else 1, bias=True)
        self.base_slim_1x3 = nn.Conv2d(32,32, kernel_size=(1,3), padding=(0,1), stride=(1,2), groups=32 if self.seperable else 1, bias=True)
        

        self.A_comv_1x1_1 = nn.Conv2d(32,32, kernel_size=1, padding=0, bias=True)
        self.A_conv_3x3_1 = nn.Conv2d(32,32, kernel_size=3, padding=1, groups=32 if self.seperable else 1, bias=True)
        self.A_conv_slim_3x1_1 = nn.Conv2d(32,32, kernel_size=(3,1), padding=(1,0), groups=32 if self.seperable else 1, bias=True)
        self.A_conv_slim_1x3_1 = nn.Conv2d(32,32, kernel_size=(1,3), padding=(0,1), groups=32 if self.seperable else 1, bias=True)
        

        self.A_comv_1x1_2 = nn.Conv2d(32,32, kernel_size=1, padding=0, bias=True)
        self.A_conv_3x3_2 = nn.Conv2d(32,32, kernel_size=3, padding=1, stride=2, groups=32 if self.seperable else 1, bias=True)
        self.A_conv_slim_3x1_2 = nn.Conv2d(32,32, kernel_size=(3,1), padding=(1,0), stride=(2,1), groups=32 if self.seperable else 1, bias=True)
        self.A_conv_slim_1x3_2 = nn.Conv2d(32,32, kernel_size=(1,3), padding=(0,1), stride=(1,2), groups=32 if self.seperable else 1, bias=True)
        

        self.B_conv_1x1_1 = nn.Conv2d(32,32, kernel_size=1, padding=0, bias=True)
        self.B_pool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        

        self.C_comv_1x1_1 = nn.Conv2d(64,64, kernel_size=1, padding=0, bias=True)
        self.C_conv_3x3_1 = nn.Conv2d(64,64, kernel_size=3, padding=1, groups=64 if self.seperable else 1, bias=True)
        self.C_conv_slim_3x1_1 = nn.Conv2d(64,64, kernel_size=(3,1), padding=(1,0), groups=64 if self.seperable else 1, bias=True)
        self.C_conv_slim_1x3_1 = nn.Conv2d(64,64, kernel_size=(1,3), padding=(0,1), groups=64 if self.seperable else 1, bias=True)
        
        self.C_comv_1x1_2 = nn.Conv2d(64,64, kernel_size=1, padding=0, bias=True)
        self.C_conv_3x3_2 = nn.Conv2d(64,64, kernel_size=3, padding=1, stride=2, groups=64 if self.seperable else 1, bias=True)
        self.C_conv_slim_3x1_2 = nn.Conv2d(64,64, kernel_size=(3,1), stride=(2,1), padding=(1,0), groups=64 if self.seperable else 1, bias=True)
        self.C_conv_slim_1x3_2 = nn.Conv2d(64,64, kernel_size=(1,3), stride=(1,2), padding=(0,1), groups=64 if self.seperable else 1, bias=True)
        

        self.D_conv_1x1_1 = nn.Conv2d(64,64, kernel_size=1, padding=0, bias=True)
        self.D_pool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        

        self.skip_1x1_2 = nn.Conv2d(64,128, kernel_size=1, padding=0, bias=True)
        self.skip_2 = nn.Conv2d(128 if self.seperable else 64,128, kernel_size=3, padding=1, stride=2, groups=128 if self.seperable else 1, bias=True)
        self.skip_slim_3x1_2 = nn.Conv2d(128 if self.seperable else 64,128, kernel_size=(3,1), padding=(1,0), stride=(2,1), groups=128 if self.seperable else 1, bias=True)
        self.skip_slim_1x3_2 = nn.Conv2d(128,128, kernel_size=(1,3), padding=(0,1), stride=(1,2), groups=128 if self.seperable else 1, bias=True)
...
class InceptionB(nn.Module): # Based on inception v4
    def __init__(
        ...
        self.D_7x7_1 = nn.Conv2d(channels//4, channels//4, kernel_size=7, stride=1, padding=3, groups=channels//4 if self.seperable else 1, bias=True)
        self.D_7x1_1 = nn.Conv2d(channels//4, channels//4, kernel_size=(7,1), stride=1, padding=(3,0), groups=channels//4 if self.seperable else 1, bias=True)
        self.D_1x7_1 = nn.Conv2d(channels//4, channels//4, kernel_size=(1,7), stride=1, padding=(0,3), groups=channels//4 if self.seperable else 1, bias=True)
        self.D_1x1_2 = nn.Conv2d(channels//4, channels//4, kernel_size=1, padding=0, bias=True)

        self.D_7x7_2 = nn.Conv2d(channels//4, channels//4, kernel_size=7, stride=1, padding=3, groups=channels//4 if self.seperable else 1, bias=True)
        self.D_7x1_2 = nn.Conv2d(channels//4, channels//4, kernel_size=(7,1), stride=1, padding=(3,0), groups=channels//4 if self.seperable else 1, bias=True)
        self.D_1x7_2 = nn.Conv2d(channels//4, channels//4, kernel_size=(1,7), stride=1, padding=(0,3), groups=channels//4 if self.seperable else 1, bias=True)
