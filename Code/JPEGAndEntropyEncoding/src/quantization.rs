...
pub fn quantize_image(dct_image : &Vec<Vec<Vec<Vec<Vec<f64>>>>>,Qf : f64){
    let quantize_mat = vec![gen_quantize_lumi_matrix(Qf),gen_quantize_chroma_matrix(Qf),gen_quantize_chroma_matrix(Qf)];
    let mut res = vec![vec![vec![];dct_image[0][0].len()];3];
    for channel in 0..dct_image.len(){
        for i in 0..dct_image[channel].len(){
            for j in 0..dct_image[channel][0].len(){
                res[channel][i].push(quantization_block(&dct_image[channel][i][j], &quantize_mat[channel]));
            }
        }
    }
    return ();
